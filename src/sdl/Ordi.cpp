...
Ordi::Ordi():jump(false),accroupi(false),attaque(false){}


void Ordi::initOrdi(int id,SDL_DisplayMode taille_window){
    perso.objet.set_vitesse(0,0);
    perso.objet.set_force(0,2000);
    perso.objet.set_rect(1200*taille_window.w/1920,400*taille_window.h/1080,6*70*taille_window.w/1920,6*80*taille_window.h/1080);
    ...
    SDL_Rect d = {1200*taille_window.w/1920,400*taille_window.h/1080,6*70*taille_window.w/1920,6*80*taille_window.h/1080};
...
void Ordi::deplacer(direction dir){
...
void Ordi::attaquer(type_attaque atq,Personnage& p,bool accroupi){
    ...
    Rectangle joueur_corps;
    if(accroupi){
        joueur_corps.x = perso.objet.rect.x + perso.objet.rect.w/3; 
        joueur_corps.y = perso.objet.rect.y + perso.objet.rect.h/3; 
        joueur_corps.w = perso.objet.rect.w/3; joueur_corps.h = 2*(perso.objet.rect.h/3);
    }
    else{
        joueur_corps.x = p.objet.rect.x + p.objet.rect.w/3; joueur_corps.y = p.objet.rect.y; 
        joueur_corps.w = p.objet.rect.w/3; joueur_corps.h = p.objet.rect.h;
    }
    ...
    case pied:
        if(!attaque && !jump){
            if(rect_attaque.collision(joueur_corps)||joueur_corps.collision(rect_attaque)){
                p.vie -= DEGAT_PIED;
            }
            attaque = true;
            im.switchAnim(6);  
...
void Ordi::update(const Rectangle& monde,SDL_DisplayMode taille_window){
    im.update();
